...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	...
	CC_SRC_PATH="./contract/farm/go/"

	echo Vendoring Go dependencies ...
	pushd ./contract/farm/go
	...
	CC_SRC_PATH="./contract/farm/java/build/install/farm"

	echo Compiling Java code ...
	pushd ./contract/farm/java
	./gradlew installDist
	popd
	...
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH="./contract/farm/typescript/"

	echo Compiling TypeScript code into JavaScript ...
	pushd ./contract/farm/typescript
	npm install
	npm run build
	popd
...
packageChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode package farm.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label farm_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode install farm.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryInstalled() {
  ...
  set +x
  cat log.txt
	PACKAGE_ID=$(sed -n "/farm_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name farm --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name farm --version ${VERSION} --sequence ${VERSION} --output json --init-required >&log.txt
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name farm $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryCommitted() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name farm >&log.txt
    res=$?
    set +x
...
chaincodeInvokeInit() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n farm $PEER_CONN_PARMS --isInit -c '{"function":"initLedger","Args":[]}' >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n farm -c '{"Args":["queryAssetsAll"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
...
# installChaincode 31
# echo "Install chaincode on peer2.org3..."
# installChaincode 32
